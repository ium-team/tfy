...
- Do not attempt to override built-in TFY summaries.
...
4. Validate strict parsing and run a TFY gateway preview.
5. Check that model-visible output is shorter when summarized, secrets are redacted, and `raw_ref` is present when summarized.
6. For repo-local rules, update `.tfy/trust.json` only after reviewing the final rule file bytes.
