...
pub(crate) enum HumanAutoActivateCmd {
    /// Install a TFY-owned bash startup hook into one explicit rcfile.
    Install(HumanAutoActivateInstallCmd),
    /// Remove the TFY-owned bash startup hook from one explicit rcfile.
    Uninstall(HumanAutoActivateUninstallCmd),
    /// Report repo marker, selected rcfile, and active shell auto-activation state.
    Status(HumanAutoActivateStatusCmd),
    /// Validate a candidate repo root before an rc hook sources repo activation content.
...
pub(crate) enum HumanCmd {
    ...
    /// Manage repo-local future-shell auto-activation through an explicit bash rc hook.
    AutoActivate {
        #[command(subcommand)]
...
fn home_rcfile() -> Result<PathBuf> {
    let home = std::env::var_os("HOME").ok_or_else(|| anyhow!("HOME is not set; pass --rcfile"))?;
...
fn auto_script_path(root: &Path) -> PathBuf {
    root.join(".tfy/human/auto-activate.bash")
}
...
pub(crate) fn enable_human_auto_activate_repo(
    ...
    let script = auto_activation_script(session, raw_dir, ledger, shell, &tfy_bin)?;
    ...
    write_auto_script(&auto_script_path(&root), &script)?;
    let marker_text = format!("{}\n", serde_json::to_string_pretty(&marker)?);
    atomic_write_checked(&auto_marker_path(&root), &marker_text)?;
    validate_auto_activate_root(&root, shell)?;
    Ok(root)
}
...
fn auto_activation_script(
    ...
    script.push('\n');
    script.push_str("# TFY-generated repo-local human auto-activation script. Do not edit; regenerated by pinned TFY validation.\n");
    script.push_str(&human_script_with_tfy_bin(
        session, raw_dir, ledger, shell, true, tfy_bin,
    )?);
...
fn validate_auto_activate_root(root: &Path, shell: &str) -> Result<HumanAutoActivateMarker> {
    ...
    if marker.shell != shell {
        bail!(
            "auto activation shell mismatch: marker={} requested={shell}",
            marker.shell
        );
    }
    ...
    let script_path = auto_script_path(&root);
...
fn auto_hook_block(tfy_bin: &Path) -> String {
    let tfy_bin = shell_quote(&tfy_bin.display().to_string());
    format!(
        r#"{AUTO_START_MARKER}
# TFY-owned Linux bash startup hook. It is silent unless TFY_HUMAN_AUTO_ACTIVATE_DEBUG=1.
...
fn execute_auto_activate_install(cmd: HumanAutoActivateInstallCmd) -> Result<()> {
    ensure_supported_shell(&cmd.shell)?;
    if cmd.apply && cmd.dry_run {
        bail!("--apply and --dry-run cannot be combined");
    }
    let rcfile = cmd.rcfile.unwrap_or(home_rcfile()?);
    ...
    let block = auto_hook_block(&tfy_bin);
    ...
    if !cmd.apply {
        ...
        println!(
            "shell=bash status=supported selected_rcfile.path={}",
            rcfile.display()
        );
    ...
    println!(
        "installed TFY human auto-activate hook rcfile={} shell=bash pinned_tfy_bin={}",
        rcfile.display(),
        tfy_bin.display()
    );
    Ok(())
...
fn execute_auto_activate_uninstall(cmd: HumanAutoActivateUninstallCmd) -> Result<()> {
    ...
    if !cmd.apply {
        ...
        println!(
            "shell=bash selected_rcfile.path={} apply=false next_action=rerun with --apply",
            rcfile.display()
        );
        return Ok(());
    }
...
fn execute_auto_activate_status(cmd: HumanAutoActivateStatusCmd) -> Result<()> {
    ...
    let marker_valid = root
        ...
        .unwrap_or(false);
    let rcfile = cmd.rcfile.or_else(|| home_rcfile().ok());
    ...
    let report = json!({
        ...
        "support_status": "linux_bash_interactive_non_login_selected_rcfile"
    });
    if cmd.json {
        ...
        println!("support_status=linux_bash_interactive_non_login_selected_rcfile");
    }
    Ok(())
}
