...
pub(crate) enum HumanCmd {
    ...
    /// Manage current-directory future-shell auto-activation through an explicit bash rc hook.
    AutoActivate {
        #[command(subcommand)]
...
fn execute_human_install(
    ...
    if dry_run || output.is_none() {
        ...
        println!("shell=bash status=supported scope=current_directory_scoped_tfy_managed_session");
