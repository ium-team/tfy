...
cargo run -p tfy-cli -- setup --ai --host cursor --dry-run # unsupported: fails closed
...
cargo run -p tfy-cli -- mcp install --target opencode --dry-run
cargo test --quiet
```
...
tfy start --agent --host cursor # unsupported: fails closed and writes no .cursor/mcp.json
...
tfy setup --ai --host cursor --apply --project # unsupported: fails closed
tfy doctor --codex
tfy smoke --mcp
