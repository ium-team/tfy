...
For AI-agent use, `tfy start --agent --host codex` writes project MCP configuration, but that only proves configuration. Launch support still requires real host invocation plus TFY raw/ledger/no-negative/positive-savings evidence. For human use, `tfy start --human` records lifecycle intent and `tfy human shell` enters an explicit TFY-managed Linux bash session where allowlisted ordinary commands route through TFY summaries; outside that managed session, `tfy shell <command>` is raw passthrough convenience and `tfy shell -- <command>` is the TFY summarizing wrapper. TFY does not claim universal terminal interception.
