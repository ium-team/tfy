...
Human and agent modes use the same raw-first/no-negative/custom command-summary pipeline. Human mode routes commands from the TFY-managed project shell. Agent mode routes commands when the AI-agent host is configured to call TFY. The default supported path is the generated TFY agent wrapper/executor route; Codex and Claude Code can now be configured through their official PreToolUse Bash hook surfaces with route evidence still gated as `configured_unverified` until a real host invocation proves raw/ledger/no-negative/positive-savings behavior. MCP remains available for advanced tool/resource integration; it is not the default command-interception story.
