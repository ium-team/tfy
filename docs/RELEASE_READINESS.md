...
Preview npm publishes must be public and use the `preview` dist-tag, not `latest`. Stable publishes use the `latest` dist-tag so `npm install -g token-fuck-you` resolves to the most stable release. Exact historical installs use standard npm version specifiers such as `token-fuck-you@0.1.1`; TFY does not use a slash form such as `token-fuck-you/v0.1.1` for npm installs. The newest intentionally unstable build is exposed through `token-fuck-you@preview`. The package must fail closed when the release archive or checksum is missing or mismatched. Release evidence should record at least `npm_package_name`, `npm_dist_tag`, `npm_bin`, `github_release_canonical`, asset platform/arch/name, and checksum file/hash.
