...
- `tfy launch-report` combines the v1 host-readiness matrix, unsupported-surface audit, required benchmark scenarios, host evidence, and current gain data. It blocks launch claims while required v1 routes lack `launch_supported` evidence or command savings data is absent. Codex can remain non-blocking `configured_but_unverified` because it is stretch-only unless real official/configurable host invocation evidence exists.
