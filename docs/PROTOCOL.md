...
- `tfy start [agent|ai|human|both] [--agent] [--human] [--host <host|all>] [--apply] [--no-apply] [--verify]` / `tfy stop [agent|ai|human|both] [--agent] [--human]` / `tfy fuckyou [agent|ai|human|both] [--agent] [--human] [--yes]` — project lifecycle intent commands backed by `.tfy/lifecycle.json`; bare interactive commands open an arrow-key TUI, while piped stdin choices stay script-compatible and missing non-TTY choices fail closed; `stop` preserves raw/shared evidence, while `fuckyou` performs scoped confirmed cleanup and preserves `.tfy/raw` by default. Agent start auto-configuration is safe-writer-only; currently `tfy start --agent`, `tfy start agent`, and `tfy start --agent --host cursor` write project `.cursor/mcp.json` unless `--no-apply` is passed, and record only `configured_unverified`. `active=true` is a derived/effective claim, not a synonym for lifecycle desire or config presence; it requires verified route evidence, raw recovery, no-negative and positive savings, and a supported host/route evidence scope.
- `tfy global start|stop|fuckyou [agent|ai|human|both] [--agent] [--human] [--yes]` — user-global lifecycle intent using `$TFY_HOME`, `$XDG_CONFIG_HOME/tfy`, or `~/.tfy`, separate from project state. `tfy use always|stop|cancel|fuckyou` are convenience aliases for the same user-global lifecycle controls (`always` also accepts the historical typo alias `alwais`). P1 global lifecycle records defaults only; host apply/verify remains project-scoped.
- `tfy status [--agent] [--human] [--json]` — lifecycle/support status with project/global separation, explicit effective project-over-global desired/configured/active state, desired_source=none in fresh projects, project_parse_error for malformed local lifecycle state, next-action guidance, and truthful route limitations.
...
- `tfy hook capabilities` / `tfy hook install --target <host> --dry-run` / `tfy hook run -- <command...>` — official-host-hook planning surface plus test shim. Hook routes are thin routers into shared gateways and are disabled/planned for real hosts until official docs and host-bound e2e evidence exist.
...
- `tfy setup --ai --host cursor --apply --project` — safe reversible project `.cursor/mcp.json` writer with backup, idempotency, and TFY-only uninstall.
...
- `tfy launch-report` combines the v1 host-readiness matrix, unsupported-surface audit, required benchmark scenarios, host evidence, and current gain data. It blocks launch claims while required v1 routes lack `launch_supported` evidence or command savings data is absent. Named hosts can remain non-blocking `config_snippet_available`, `applied_unverified`, `verified_local_mcp`, or `verified_host_invocation`; lifecycle status can also be `configured_unverified` for safe config writes that are not launch-report evidence until real host invocation plus host-bound config/ledger/raw/no-negative/positive-savings evidence exists.
...
`tfy hook` is a dry-run/planning plus test-shim surface for future official host hooks. It fails closed with `TFY_HOOK_DISABLE=1`; real host targets remain `planned_discovery` or `unsupported` until official docs, kill-switch/uninstall posture, and e2e route evidence exist.
