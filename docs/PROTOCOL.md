...
- `tfy start [agent|ai|human|both] [--agent] [--human] [--host <host|all>] [--apply] [--verify]` / `tfy stop [agent|ai|human|both] [--agent] [--human]` / `tfy fuckyou [agent|ai|human|both] [--agent] [--human] [--yes]` — project lifecycle intent commands backed by `.tfy/lifecycle.json`; `stop` preserves raw/shared evidence, while `fuckyou` performs scoped confirmed cleanup and preserves `.tfy/raw` by default. Host apply is explicit and safe-writer-only; currently `tfy start agent --host cursor --apply` writes project `.cursor/mcp.json` but records only `applied_unverified`.
- `tfy global start|stop|fuckyou [agent|ai|human|both] [--agent] [--human] [--yes]` — user-global lifecycle intent using `$TFY_HOME`, `$XDG_CONFIG_HOME/tfy`, or `~/.tfy`, separate from project state. P1 global lifecycle records defaults only; host apply/verify remains project-scoped.
