...
- `tfy smoke --all --json` runs generic-shell, agent-wrapper, and MCP smokes and emits ledger paths that can be supplied to `tfy launch-report --all --ledger ...`; without separate artifact-backed host setup/real-invocation and overhead evidence these routes are only `verified_local_smoke`.
...
- `tfy launch-report --json` exposes the `mcp_stdio`, `tfy_agent_adapter`, `generic_shell`, and `codex` readiness matrix, blocks release while required v1 hosts (`mcp_stdio`, `tfy_agent_adapter`, `generic_shell`) lack `launch_supported` evidence, promotes local smoke only to `verified_local_smoke`, requires artifact-backed `--host-evidence` JSON plus overhead evidence before `launch_supported`, keeps Codex non-blocking but `configured_but_unverified` without real host invocation evidence, lists unsupported provider/editor/private-hook/universal-terminal paths, reports exact byte savings plus conservative token proxy estimates, and includes the seven required launch benchmark scenario names.
