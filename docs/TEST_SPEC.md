...
- `tfy start --human --auto-activate` creates/refreshes `.tfy/human/auto-activate.json` and deterministic `.tfy/human/auto-activate.bash`; plain `tfy start --human` does not create future-shell auto-activation state.
