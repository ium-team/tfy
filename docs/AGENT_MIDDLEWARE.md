...
If a host publishes an official command hook, TFY may use that hook only after docs-backed support and route evidence exist. Otherwise the supported generic path is the wrapper/executor route.
...
The host still decides to route through MCP. TFY does not secretly intercept every Codex command. The explicit MCP setup path is:
