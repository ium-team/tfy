...
- Human lifecycle: `tfy start --human` records managed-session intent only and always keeps `active=false` plus `ordinary_terminal_interception=false` until real managed-session evidence exists. On supported Linux bash v1 hosts it reports `session_wrapper_available=true` and `managed_session_available=true`; on unsupported non-Linux platforms it reports `managed_session_available=false` with `support_status=managed_session_unsupported_platform`. `tfy human shell` is the explicit Linux bash managed session where allowlisted ordinary commands route through the same Tool Gateway summaries, and `tfy human install --dry-run|--output <path>` generates the TFY-owned sourceable script. Ordinary terminals outside that managed session are not globally intercepted.
