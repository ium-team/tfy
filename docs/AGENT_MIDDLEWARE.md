...
Setup remains `configured_unverified` until a real host invocation creates route-bound evidence. `tfy start --agent --host codex` and `tfy start --agent --host claude-code` remain explicit/narrow setup paths. Cursor and other hosts use explicit host setup, the wrapper/executor route, or MCP until a stronger official command route is proven.
