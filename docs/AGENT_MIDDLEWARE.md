...
- Configured AI-agent wrapper: implemented through `tfy agent run`; lifecycle `tfy start --agent` now auto-configures implemented safe project host routes (Cursor MCP in v1) unless `--no-apply` is passed, but it does not prove host invocation or launch support; effective `active=true` is derived only from verified host/route evidence and savings, not config presence.
