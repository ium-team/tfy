...
- Bare project-local `tfy start --agent` is the simple product path: it must prepare the generic TFY agent wrapper plus Codex and Claude Code official project hook routes by default, while Cursor is explicitly unsupported and broader hosts remain outside the default path.
