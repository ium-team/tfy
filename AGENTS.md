...
- Bare project-local `tfy start --agent` is the simple product path: it must prepare the generic TFY agent wrapper plus Codex and Claude Code official project hook routes by default, while Cursor and broader hosts remain explicit `--host ...` or `--host all` choices.
...
- Do not implement that default by aliasing it to `--host all`; this would silently expand scope to Cursor/guidance routes.
